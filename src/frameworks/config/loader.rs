...
fn hint_for_path(field_path: &str) -> &'static str {
    if field_path.starts_with("blocklists") || field_path.starts_with("allowlists") {
        ...
        "Check each zone includes 'zone' and exactly one mode: either 'servers' (forwarding) or 'zone_source' (authoritative JSON from file/http/https). Optional flags are 'enabled', 'bypass_filter', and 'fallback_to_default_resolvers'; optional 'zone_source_check_interval' is for URL sources; zone-level 'strategy' supports round_robin, random, or failover."
