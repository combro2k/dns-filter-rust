...
- **Init-system wiring for `status`**: `dns-filter status` now also delegates to systemd/OpenRC under default settings for service-managed installs. Added `--direct` to force the control-socket statistics path when needed.
